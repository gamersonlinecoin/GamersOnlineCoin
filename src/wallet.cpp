...
// gamersonlinecoin: total coins staked (non-spendable until maturity)
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-gamersonlinecoin-address
...
// GamersOnlineCoin: get current stake weight
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, int64_t nValue, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    // Parse gamersonlinecoin address
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CgamersonlinecoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CgamersonlinecoinAddress(address).ToString());
}
...
// gamersonlinecoin: check 'spent' consistency between wallet and txindex
// gamersonlinecoin: fix wallet spent state according to txindex
...
// gamersonlinecoin: disable transaction (only for coinstake)
