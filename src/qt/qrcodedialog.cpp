...
{
    QString ret = QString("gamersonlinecoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non GOC unit input in lnReqAmount, we generate the URI with GOC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(gamersonlinecoinUnits::format(gamersonlinecoinUnits::GOC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
