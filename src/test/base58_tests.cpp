...
BOOST_AUTO_TEST_CASE(base58_keys_valid_parse)
{
    ...
    std::vector<unsigned char> result;
    CgamersonlinecoinSecret secret;
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if(isPrivkey)
        {
            ...
            // Note: CgamersonlinecoinSecret::SetString tests isValid, whereas CgamersonlinecoinAddress does not!
...
BOOST_AUTO_TEST_CASE(base58_keys_valid_gen)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        else
        {
            ...
            CgamersonlinecoinAddress addrOut;
            BOOST_CHECK_MESSAGE(boost::apply_visitor(CgamersonlinecoinAddressVisitor(&addrOut), dest), "encode dest: " + strTest);
            BOOST_CHECK_MESSAGE(addrOut.ToString() == exp_base58string, "mismatch: " + strTest);
        }
    }

    // Visiting a CNoDestination must fail
    CgamersonlinecoinAddress dummyAddr;
    CTxDestination nodest = CNoDestination();
    BOOST_CHECK(!boost::apply_visitor(CgamersonlinecoinAddressVisitor(&dummyAddr), nodest));
